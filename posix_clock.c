...
void *delay_test(void *threadID)
{
    ...
    for(idx = 0; idx < TEST_ITERATIONS; ++idx)
	{
		...
		//printing value of clock_gettime()
		printf("Start Timestamp: %ld (nanoseconds), %d (seconds)\n", 
			   rtclk_start_time.tv_nsec, rtclk_start_time.tv_sec);
		...
		printf("Stop Timestamp: %ld (nanoseconds), %d (seconds)\n", 
		rtclk_stop_time.tv_nsec, rtclk_stop_time.tv_sec);
...
void end_delay_test(void)
{
  ...
  printf("MY_CLOCK clock DT seconds = %ld, msec=%ld, usec=%ld, nsec=%ld, sec=%6.9lf\n", 
		 ...
		 rtclk_dt.tv_nsec/1000, rtclk_dt.tv_nsec, real_dt);
